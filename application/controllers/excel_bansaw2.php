...
class Excel_bansaw2 extends CI_Controller {
	...
	public function upload(){
		...
		try {
			...
			for ($row = 2; $row <= $highestRow; $row++){                  //  Read a row of data into an array
				...
				$insert = $this->db->insert("bansaw2",$data);
				// delete_files($media['file_path']);

				$this->Bongkar_log_model->bongkar_log_bansaw2($data['bpk_no_bpk'], $data['grade_grade'],$data['diameter']);
