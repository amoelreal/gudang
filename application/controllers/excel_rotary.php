...
class Excel_rotary extends CI_Controller {
	...
	public function upload(){
		...
		try {
			...
			for ($row = 2; $row <= $highestRow; $row++){                  //  Read a row of data into an array
				...
				$insert = $this->db->insert("log_rotary",$data);
				// delete_files($media['file_path']);
				if($insert){
				$this->Bongkar_log_model->bongkar_log_rotary($data['bpk_no_bpk'], $data['grade_grade'],$data['diameter']);
